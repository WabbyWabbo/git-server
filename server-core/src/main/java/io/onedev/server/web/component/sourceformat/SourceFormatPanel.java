...
public class SourceFormatPanel extends Panel {
	...
	public SourceFormatPanel(String id, @Nullable OptionChangeCallback indentTypeChangeCallback,
		...
		/*
		 * cookie = request.getCookie(COOKIE_LINE_WRAP_MODE); if (cookie != null)
		 * lineWrapMode = cookie.getValue().replace('-', ' '); else lineWrapMode =
		 * "Soft wrap";
		 */
	}
	...
	protected void onInitialize() {
		 ...
		 * 
		 * @Override public String getObject() { return lineWrapMode; }
		 * 
		 * @Override public void setObject(String object) { lineWrapMode = object; }
		 * 
		 * }, Lists.newArrayList("No wrap", "Soft wrap")).add(new OnChangeAjaxBehavior()
		 * {
		 * 
		 * @Override protected void onUpdate(AjaxRequestTarget target) { WebResponse
		 * response = (WebResponse) RequestCycle.get().getResponse(); Cookie cookie =
		 * new Cookie(COOKIE_LINE_WRAP_MODE, lineWrapMode.replace(' ', '-'));
